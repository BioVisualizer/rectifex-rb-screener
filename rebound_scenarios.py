...
class ClassicOversoldScenario(BaseScenario):
    ...
    def _check_signal(self, data: pd.DataFrame) -> tuple[bool, float, float, float]:
        latest_data = data.iloc[-1]; current_price = latest_data['Close']; rsi = latest_data['RSI']
        sma200 = latest_data['SMA200']; low90d = latest_data['Low90D']
        if pd.isna(current_price) or pd.isna(rsi): return False, np.nan, np.nan, np.nan
        ...
        is_candidate_A = rsi < config.RSI_OVERSOLD_STRONG; is_candidate_B = False
        if rsi < config.RSI_OVERSOLD_WEAK:
            if 0 <= dist_to_sma <= config.SUPPORT_PROXIMITY_THRESHOLD or 0 <= dist_to_low <= config.SUPPORT_PROXIMITY_THRESHOLD:
                is_candidate_B = True
    ...
    def _calculate_score(self, rsi: float, dist_to_sma: float, dist_to_low: float) -> tuple[int, int, int]:
        ...
        rsi_score = int(min(100, rsi_score)); prox_dist = np.inf
        if dist_to_sma >= 0: prox_dist = min(prox_dist, dist_to_sma)
        if dist_to_low >= 0: prox_dist = min(prox_dist, dist_to_low)
        if prox_dist > config.PROXIMITY_SCORE_CEILING: proximity_score = 0
        else: proximity_score = (config.PROXIMITY_SCORE_CEILING - prox_dist) * (100 / config.PROXIMITY_SCORE_CEILING)
...
class MeanReversionScenario(BaseScenario):
    ...
    def _calculate_score(self, percent_below_band: float) -> int:
        score_percent = min(percent_below_band, 5.0)
    ...
    def run(self, stock_data: pd.DataFrame, fundamental_data: Dict, stock_info: Dict) -> Optional[ReboundCandidate]:
        ...
        if pd.isna(current_price) or pd.isna(lower_band): return None
...
class VolatilitySqueezeScenario(BaseScenario):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.squeeze_period = 125
    def _prepare_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        if df is None or df.empty or len(df) < self.squeeze_period: return pd.DataFrame()
        df['BB_Upper'], df['BB_Middle'], df['BB_Lower'] = calculate_bollinger_bands(df['Close'])
    ...
    def _calculate_score(self, current_width: float, min_width: float) -> int:
        if pd.isna(min_width) or min_width <= 0: return 0
        proximity = (current_width - min_width) / min_width
        score = 100 - (proximity / 0.1 * 100)
        return int(max(0, min(100, score)))
    def run(self, stock_data: pd.DataFrame, fundamental_data: Dict, stock_info: Dict) -> Optional[ReboundCandidate]:
        if len(stock_data) < self.squeeze_period: return None
        stock_data = self._prepare_dataframe(stock_data)
        if stock_data.empty: return None
        latest = stock_data.iloc[-1]
        current_width, min_width = latest.get('BB_Width'), latest.get('BB_Width_Min')
        if not (pd.notna(current_width) and pd.notna(min_width) and current_width <= min_width * 1.1): return None
        self._emit_progress(f"!!! {stock_info['ticker']} is a potential '{self.name}' candidate.")
...
class MomentumBreakoutScenario(BaseScenario):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.breakout_period = 252
    def _prepare_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        if df is None or df.empty or len(df) < self.breakout_period: return pd.DataFrame()
    ...
    def run(self, stock_data: pd.DataFrame, fundamental_data: Dict, stock_info: Dict) -> Optional[ReboundCandidate]:
        if stock_data is None or len(stock_data) < self.breakout_period: return None
        stock_data = self._prepare_dataframe(stock_data)
        if stock_data.empty: return None
        latest = stock_data.iloc[-1]
        ...
        if not (pd.notna(current_price) and pd.notna(high_52w) and pd.notna(current_volume) and pd.notna(avg_volume) and avg_volume > 0): return None
        if not (current_price > high_52w and current_volume > avg_volume * 1.5): return None
        self._emit_progress(f"!!! {stock_info['ticker']} is a potential '{self.name}' candidate.")
...
class GoldenCrossScenario(BaseScenario):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.recency_days = 5; self.min_data_days = 200
    def _prepare_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        if df is None or df.empty or len(df) < self.min_data_days: return pd.DataFrame()
    ...
    def run(self, stock_data: pd.DataFrame, fundamental_data: Dict, stock_info: Dict) -> Optional[ReboundCandidate]:
        if stock_data is None or len(stock_data) < self.min_data_days: return None
        stock_data = self._prepare_dataframe(stock_data)
        if stock_data.empty or 'SMA50' not in stock_data.columns or 'SMA200' not in stock_data.columns: return None
        ...
        for i in range(len(recent_data) - 1, 0, -1):
            ...
            if pd.notna(today['SMA50']) and pd.notna(today['SMA200']) and pd.notna(yesterday['SMA50']) and pd.notna(yesterday['SMA200']):
                if today['SMA50'] > today['SMA200'] and yesterday['SMA50'] <= yesterday['SMA200']:
                    days_ago = len(recent_data) - 1 - i; cross_found = True; break
        if not cross_found: return None
        self._emit_progress(f"!!! {stock_info['ticker']} is a potential '{self.name}' candidate.")
...
class HighQualityDividendScenario(BaseScenario):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.min_yield = 0.03; self.max_payout_ratio = 0.7; self.max_debt_to_equity = 1.0
    ...
    def run(self, stock_data: pd.DataFrame, fundamental_data: Dict, stock_info: Dict) -> Optional[ReboundCandidate]:
        if not (fundamental_data and 'metrics' in fundamental_data): return None
        fund_metrics = fundamental_data['metrics']; div_yield = fund_metrics.get('dividendYield'); payout = fund_metrics.get('payoutRatio'); debt = fund_metrics.get('debtToEquity')
        if not all(v is not None for v in [div_yield, payout, debt]): return None
        if not (div_yield >= self.min_yield and 0 < payout < self.max_payout_ratio and debt < self.max_debt_to_equity): return None
        self._emit_progress(f"!!! {stock_info['ticker']} is a potential '{self.name}' candidate.")
...
class FundamentalDivergenceScenario(BaseScenario):
    ...
    def _prepare_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        if df is None or df.empty or len(df) < self.scan_period: return pd.DataFrame()
        df['SMA50'] = calculate_sma(df['Close'], 50); df['SMA200'] = calculate_sma(df['Close'], 200)
        ...
        if not recent_prices.empty:
            ...
            if last_price > 0: df['PriceRange120D'] = (max_price - min_price) / last_price
        return df
    def run(self, stock_data: pd.DataFrame, fundamental_data: Dict, stock_info: Dict) -> Optional[ReboundCandidate]:
        if stock_data is None or len(stock_data) < self.scan_period: return None
        sector_stats = {}
        ...
        if fundamental_data and 'metrics' in fundamental_data:
            prelim_fundamental_score, _ = compute_fundamental_score(fundamentals=fundamental_data['metrics'], sector=fundamental_data.get('sector', 'N/A'), sector_stats=sector_stats, weights=DIVERGENCE_SCENARIO_FUNDAMENTAL_WEIGHTS)
        if prelim_fundamental_score < 45: return None
        stock_data = self._prepare_dataframe(stock_data)
        if stock_data.empty or 'SMA50' not in stock_data.columns or 'SMA200' not in stock_data.columns: return None
        latest = stock_data.iloc[-1]
        if pd.isna(latest['SMA50']) or pd.isna(latest['SMA200']) or latest['SMA200'] <= 0: return None
...
class QualityPullbackScenario(BaseScenario):
    ...
    def _prepare_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        if df is None or df.empty: return pd.DataFrame()
        df['RSI'] = calculate_rsi(df['Close'], config.RSI_PERIOD); df['SMA50'] = calculate_sma(df['Close'], 50); df['SMA200'] = calculate_sma(df['Close'], config.SMA_SUPPORT_PERIOD)
        return df
    def run(self, stock_data: pd.DataFrame, fundamental_data: Dict, stock_info: Dict) -> Optional[ReboundCandidate]:
        if stock_data is None or len(stock_data) < 200: return None
        stock_data = self._prepare_dataframe(stock_data); latest = stock_data.iloc[-1]
        if not (pd.notna(latest['Close']) and pd.notna(latest['SMA50']) and pd.notna(latest['SMA200']) and latest['Close'] > latest['SMA200'] and latest['SMA50'] > latest['SMA200']): return None
        current_price, sma50 = latest['Close'], latest['SMA50']; dist_to_sma50 = -1
        if pd.notna(current_price) and pd.notna(sma50) and sma50 > 0:
            dist_to_sma50 = abs((current_price - sma50) / sma50) * 100
            if dist_to_sma50 > 3.0: return None
        else: return None
        self._emit_progress(f"!!! {stock_info['ticker']} is a potential '{self.name}' candidate.")
...
class ScenarioRunner:
    ...
    SCENARIO_CLASS_MAP = {
        "ClassicOversoldScenario": ClassicOversoldScenario, "QualityPullbackScenario": QualityPullbackScenario,
