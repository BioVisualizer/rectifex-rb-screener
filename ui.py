...
from PyQt6.QtWidgets import (
    ...
    QStackedWidget, QButtonGroup, QDialog, QDialogButtonBox, QCheckBox, QStyledItemDelegate
)
...
class AnalysisWorker(QObject):
    ...
    def run(self):
        try:
            runner = ScenarioRunner(progress_callback=self.signals.progress, progress_percent_callback=self.signals.progress_percent, is_cancelled_callback=lambda: self._is_cancelled)
...
# --- Table View Models & Delegates ---

def create_sparkline(price_data: pd.Series) -> Optional[QPixmap]:
    if price_data is None or price_data.empty or len(price_data) < 2: return None
    start_price, end_price = price_data.iloc[0], price_data.iloc[-1]
    line_color = 'green' if end_price >= start_price else 'red'
    fig = Figure(figsize=(2.5, 0.5), dpi=72)
    fig.patch.set_alpha(0); ax = fig.add_subplot(111); ax.patch.set_alpha(0)
    price_data.plot(ax=ax, color=line_color, linewidth=1.5)
    ...
    try:
        ...
        logging.error(f"Failed to create sparkline image: {e}")
        return None
    finally:
        plt.close(fig)
...
class SparklineDelegate(QStyledItemDelegate):
    def paint(self, painter: QPainter, option, index):
        item_data = index.data(role=Qt.ItemDataRole.DisplayRole)
        if isinstance(item_data, QPixmap) and not item_data.isNull():
            rect = option.rect
            target_rect = item_data.rect()
            target_rect.moveCenter(rect.center())
            painter.drawPixmap(target_rect, item_data)
        else:
...
class PandasModel(QAbstractTableModel):
    def __init__(self, data=pd.DataFrame(), candidates_data=[], parent=None):
        ...
        self.tooltips = {
            self.tr("Ticker"): self.tr("The stock symbol for the company."),
            self.tr("Name"): self.tr("The name of the company."),
            self.tr("Price"): self.tr("The most recent closing price of the stock."),
            self.tr("Change %"): self.tr("The percentage change in price over the last day. (Not yet implemented)"),
            self.tr("Rebound Score"): self.tr("A composite score (0-100) indicating the overall potential for a rebound. Higher is better."),
            self.tr("Tech. Score"): self.tr("A score (0-100) based on technical indicators like price action, moving averages, and momentum."),
            self.tr("Fund. Score"): self.tr("A score (0-100) based on fundamental data like revenue growth, earnings, and debt."),
            self.tr("ROE"): self.tr("Return on Equity. A measure of a company's profitability in relation to the equity held by shareholders. Higher is generally better."),
            self.tr("Sparkline"): self.tr("A small chart showing the recent price trend of the stock over the last 30 days.")
        }
    ...
    def data(self, index, role=Qt.ItemDataRole.DisplayRole):
        ...
        if role == Qt.ItemDataRole.DisplayRole:
            return "" if isinstance(value, QPixmap) else str(value)
        ...
        if role == Qt.ItemDataRole.BackgroundRole:
            if "Rebound Score" in self._data.columns:
                score = self._data.iloc[row][self.tr("Rebound Score")]
                if score > 80: return QColor("#d4edda")
                elif score > 60: return QColor("#fff3cd")
        if role == Qt.ItemDataRole.ToolTipRole and column_name == self.tr("Rebound Score"):
            ...
            if row < len(self.candidates_data):
                ...
                tech_w, fund_w, mark_w = [DEFAULT_REBOUND_SCORE_WEIGHTS[k] * 100 for k in ['tech', 'fund', 'market']]
                breakdown_lines = [f"\n\n--- {self.tr('Score Composition')} ---", f"- {self.tr('Technical Score')}: {candidate.technical_score} ({self.tr('Weight')}: {tech_w:.0f}%)", f"- {self.tr('Fundamental Score')}: {candidate.fundamental_score} ({self.tr('Weight')}: {fund_w:.0f}%)", f"- {self.tr('Market Context')}: {candidate.market_context_score} ({self.tr('Weight')}: {mark_w:.0f}%)"]
                if candidate.score_breakdown:
                    breakdown_lines.append(f"\n--- {self.tr('Details')} ---")
                    for key, val in candidate.score_breakdown.items():
                        readable_key = key.replace('_sub_score', '').replace('_', ' ').title()
                        breakdown_lines.append(f"- {readable_key}: {val}")
                return base_tooltip + "\n".join(breakdown_lines)
            return base_tooltip
        return None
    def headerData(self, section, orientation, role=Qt.ItemDataRole.DisplayRole):
        if orientation == Qt.Orientation.Horizontal:
            column_name = str(self._data.columns[section])
            if role == Qt.ItemDataRole.DisplayRole: return column_name
            if role == Qt.ItemDataRole.ToolTipRole: return self.tooltips.get(column_name, column_name)
        return None
    def get_dataframe(self): return self._data

class CustomSortProxyModel(QSortFilterProxyModel):
    def lessThan(self, left, right):
        col_name = self.sourceModel().headerData(self.sortColumn(), Qt.Orientation.Horizontal, Qt.ItemDataRole.DisplayRole)
        numeric_sort_columns = [self.tr('Rebound Score'), self.tr('Tech. Score'), self.tr('Fund. Score'), self.tr('Price')]
        if col_name in numeric_sort_columns:
...
# ... (ChartWindow, ToastNotification, ScanCategoryCard, and SettingsDialog remain mostly unchanged but with tr() wrappers) ...
class ChartWindow(QWidget):
    def __init__(self, candidate: ReboundCandidate):
        super().__init__(); self.candidate = candidate; self.setWindowTitle(f"Chart for {self.candidate.ticker} - {config.APP_NAME}")
        self.setGeometry(150, 150, 950, 750); layout = QVBoxLayout(self); self.setLayout(layout)
        self.figure = Figure(figsize=(10, 8), facecolor=self.palette().window().color().name()); self.canvas = FigureCanvas(self.figure)
        layout.addWidget(self.canvas); self.plot_stock_data(candidate)
    def plot_stock_data(self, candidate: ReboundCandidate):
        try:
            self.figure.clear()
            if candidate.history_df is None or candidate.history_df.empty:
                ax = self.figure.add_subplot(111); ax.text(0.5, 0.5, f"Historical data for {candidate.ticker} not found.", horizontalalignment='center', verticalalignment='center'); self.canvas.draw(); return
            history_df = candidate.history_df.copy()
            if not history_df.empty and config.CHART_HISTORY_MONTHS > 0:
                plot_data = history_df.loc[history_df.index.max() - pd.DateOffset(months=config.CHART_HISTORY_MONTHS):].copy()
            else: plot_data = history_df.copy()
            ...
            if pd.notna(highest_high) and pd.notna(lowest_low) and highest_high > lowest_low:
                ...
                fib_levels = {23.6: highest_high - (price_range * 0.236), 38.2: highest_high - (price_range * 0.382), 50.0: highest_high - (price_range * 0.5), 61.8: highest_high - (price_range * 0.618), 78.6: highest_high - (price_range * 0.786), 0.0: highest_high, 100.0: lowest_low}
            ...
            price_ax = self.figure.add_subplot(gs[0, 0]); volume_ax = self.figure.add_subplot(gs[1, 0], sharex=price_ax)
            rsi_ax = self.figure.add_subplot(gs[2, 0], sharex=price_ax); macd_ax = self.figure.add_subplot(gs[3, 0], sharex=price_ax)
            for ax in [price_ax, volume_ax, rsi_ax]: ax.tick_params(axis='x', which='both', bottom=False, top=False, labelbottom=False)
            add_plots = [mpf.make_addplot(plot_data['RSI'], ax=rsi_ax), mpf.make_addplot(macd_line, ax=macd_ax), mpf.make_addplot(signal_line, ax=macd_ax, color='red'), mpf.make_addplot(macd_hist, type='bar', ax=macd_ax, color='grey', alpha=0.5)]
            ...
            if fib_levels:
                transform = price_ax.get_yaxis_transform(); sorted_levels = sorted(fib_levels.items(), key=lambda item: item[1], reverse=True); last_y = float('inf')
            ...
            rsi_ax.axhline(70, color='red', linestyle='--', linewidth=0.7, alpha=0.8); rsi_ax.axhline(30, color='green', linestyle='--', linewidth=0.7, alpha=0.8); rsi_ax.set_ylim(0, 100)
            ...
            self.figure.subplots_adjust(top=0.90, bottom=0.08, left=0.08, right=0.95, hspace=0.15); self.canvas.draw()
        except Exception as e:
            logging.error(f"Failed to plot chart for {candidate.ticker}: {e}", exc_info=True); self.figure.clear()
            ...
            self.canvas.draw(); QMessageBox.critical(self, self.tr("Chart Error"), self.tr("Could not generate chart for {ticker}:\n\n{error}").format(ticker=candidate.ticker, error=e))

class ToastNotification(QFrame):
    retry = pyqtSignal()
    def __init__(self, parent=None):
        super().__init__(parent); self.setObjectName("ToastNotification"); self.setFrameShape(QFrame.Shape.StyledPanel)
        ...
        layout = QVBoxLayout(self); title_label = QLabel(f"<b>{self.tr('Scan Failed')}</b>"); title_label.setStyleSheet("font-size: 14px;")
        self.message_label = QLabel(self.tr("An error occurred. Please try again.")); button_layout = QHBoxLayout()
        retry_button = QPushButton(self.tr("Retry")); details_button = QPushButton(self.tr("Show Details"))
    ...
    def show_details(self):
        msg_box = QMessageBox(self); msg_box.setIcon(QMessageBox.Icon.Critical); msg_box.setWindowTitle(self.tr("Error Details"))
        msg_box.setText(self.tr("An unexpected error occurred during the scan.")); msg_box.setDetailedText(self.detailed_error)
...
class SettingsDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent); self.setWindowTitle(self.tr("Settings")); self.setMinimumWidth(400)
        layout = QVBoxLayout(self); lang_layout = QHBoxLayout(); lang_label = QLabel(self.tr("UI Language:"))
        self.lang_combo = QComboBox(); self.lang_combo.addItem("English", "en"); self.lang_combo.addItem("German (Deutsch)", "de"); self.lang_combo.addItem("Spanish (Español)", "es")
        ...
        layout.addSpacing(20); clear_cache_button = QPushButton(self.tr("Clear Scanner Cache"))
        clear_cache_button.clicked.connect(self.clear_cache); layout.addWidget(clear_cache_button); layout.addStretch()
    ...
    def accept(self):
        ...
        QMessageBox.information(self, self.tr("Restart Required"), self.tr("The language change will take effect the next time you start the application.")); super().accept()
    def clear_cache(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                shutil.rmtree(config.CACHE_DIR); config.CACHE_DIR.mkdir(parents=True, exist_ok=True); QMessageBox.information(self, self.tr("Success"), self.tr("Cache successfully cleared."))
            except Exception as e: QMessageBox.critical(self, self.tr("Cache Error"), self.tr("Failed to clear cache: {error}").format(error=e))

class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__(); self.setWindowTitle(config.APP_NAME); self.setGeometry(100, 100, 1200, 800)
        icon = QIcon.fromTheme("com.rectifex.GlobalReboundScreener");
        if not icon.isNull(): self.setWindowIcon(icon)
        self.chart_windows = []; self.results_df = pd.DataFrame(); self.activeScan = None; self.scan_cards = []
        self.strategy_button_group = QButtonGroup(self); self.strategy_button_group.setExclusive(True)
        ...
        self.run_scan_button = QPushButton(self.tr(" Run Scan")); self.run_scan_button.setObjectName("btn-run-scan"); self.run_scan_button.setIcon(QIcon.fromTheme("system-search"))
        ...
        inputs_layout = QHBoxLayout(); inputs_layout.setContentsMargins(20, 0, 20, 0); inputs_layout.addWidget(QLabel(self.tr("Single Ticker (Optional):")))
        ...
        inputs_layout.addWidget(self.single_ticker_input); inputs_layout.addWidget(QLabel(self.tr("Filter Results:"))); self.filter_results_input = QLineEdit()
        ...
        inputs_layout.addWidget(self.filter_results_input); toolbar_layout.addLayout(inputs_layout); toolbar_layout.addStretch()
        ...
        toolbar_layout.addWidget(self.manage_watchlists_button); toolbar_layout.addWidget(self.export_button); toolbar_layout.addWidget(self.help_button); toolbar_layout.addWidget(self.settings_button)
        top_level_layout.addLayout(toolbar_layout); main_splitter = QSplitter(Qt.Orientation.Horizontal); self.selection_pane = QWidget()
        self.selection_pane.setObjectName("selection-pane"); selection_pane_layout = QVBoxLayout(self.selection_pane)
        selection_pane_layout.setContentsMargins(0,0,0,0); scroll_area = QScrollArea(); scroll_area.setWidgetResizable(True)
        scroll_area.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff); selection_pane_layout.addWidget(scroll_area)
        card_container = QWidget(); self.card_layout = QVBoxLayout(card_container); self.card_layout.setSpacing(5); self.card_layout.addStretch()
        scroll_area.setWidget(card_container); self.populate_strategy_cards(); main_splitter.addWidget(self.selection_pane)
        ...
        main_content_layout.setContentsMargins(0, 0, 0, 0); self.stacked_widget = QStackedWidget(); main_content_layout.addWidget(self.stacked_widget)
        ...
        headline = QLabel(self.tr("Welcome! Let's Find Your Next Investment.")); headline.setStyleSheet("font-size: 24px; font-weight: bold;")
        ...
        instructions.setStyleSheet("font-size: 14px;"); self.demo_scan_button = QPushButton(self.tr("Run Demo Scan")); self.demo_scan_button.setFixedWidth(150)
        ...
        self.context_pane = QWidget(); self.context_pane.setObjectName("context-pane")
        ...
        main_splitter.setSizes([int(self.width() * 0.25), int(self.width() * 0.50), int(self.width() * 0.25)])
    ...
    def on_ticker_selected(self, index):
        ...
        candidate = self.all_candidates_data[source_index.row()]; self.context_stack.setCurrentIndex(1)
        ...
        self.ticker_detail_layout.addWidget(loading_label); self.detail_thread = QThread(); self.detail_worker = TickerDetailWorker(candidate.ticker)
        self.detail_worker.moveToThread(self.detail_thread); self.detail_thread.started.connect(self.detail_worker.run)
        self.detail_worker.signals.result.connect(self._populate_context_pane); self.detail_worker.signals.error.connect(self.scan_error)
        self.detail_worker.signals.finished.connect(self.detail_thread.quit); self.detail_worker.signals.finished.connect(self.detail_worker.deleteLater)
        self.detail_thread.finished.connect(self.detail_thread.deleteLater); self.detail_thread.start()
    def _populate_context_pane(self, info: dict):
        ...
        title = QLabel(f"<b>{info.get('shortName', info.get('symbol'))}</b> ({info.get('symbol')})"); title.setStyleSheet("font-size: 16px; margin-bottom: 5px;")
        def format_market_cap(mc):
            ...
            return f"${mc}"
        pe_ratio = f"{info.get('trailingPE'):.2f}" if info.get('trailingPE') else "N/A"; div_yield = f"{info.get('dividendYield')*100:.2f}%" if info.get('dividendYield') else "N/A"
        ...
        metrics_label = QLabel(metrics_text); bio_title = QLabel(f"<b>{self.tr('Company Bio')}</b>"); bio_title.setStyleSheet("color: #005a9e; margin-top: 10px;")
...
<p>{self.tr("This tool is for informational purposes only and does not constitute financial advice. Always conduct your own thorough research before making any investment decisions.")}</p>"""
    ...
    def display_results(self, results):
        ...
        for r in results:
            ...
            sparkline_pixmap = None
            if r.history_df is not None: sparkline_pixmap = create_sparkline(r.history_df['Close'].tail(30))
            res_dict = {
                ...
                self.tr("Fund. Score"): r.fundamental_score, self.tr("ROE"): roe_str, self.tr("Sparkline"): sparkline_pixmap if sparkline_pixmap else ""
            }
        ...
        try:
            sparkline_col_index = self.results_df.columns.get_loc(self.tr("Sparkline"))
            self.table_view.setItemDelegateForColumn(sparkline_col_index, SparklineDelegate(self))
            self.table_view.resizeColumnToContents(sparkline_col_index)
        except KeyError: logging.warning("Could not find 'Sparkline' column to set delegate.")
